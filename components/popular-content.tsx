...
export const PopularProducts = ({ data, showNutrition = false }: PopularProductsProps) => {
    ...
    return (
        <Card className="w-full max-h-[340px] rounded-md bg-white shadow-lg border-none flex flex-col items-center justify-center relative pt-24 md:pt-36">
            ...
            <div className="w-full flex items-center justify-center gap-2 flex-wrap px-2 mt-4">
                {data.cuisine && (
                    <div className="rounded-md bg-emerald-50 px-2 py-[2px] text-[10px] font-semibold capitalize">
                        {data.cuisine}
                    </div>
                )}
                {data.category && (
                    <div className="rounded-md bg-blue-50 px-2 py-[2px] text-[10px] font-semibold capitalize">
                        {data.category}
                    </div>
                )}
                {data.size && (
                    <div className="rounded-md bg-yellow-50 px-2 py-[2px] text-[10px] font-semibold capitalize">
                        {data.size}
                    </div>
                )}
                {data.kitchen && (
            ...
            <Button 
                ...
                variant={"ghost"} 
                className="absolute top-0 left-0 rounded-tl-none rounded-tr-lg rounded-br-none p-2 px-3"
            >
                <IsLikedIcon className={`w-5 h-5 ${isLiked ? "text-red-600" : "text-black"}`} />
            </Button>
        </Card>
    )
}
